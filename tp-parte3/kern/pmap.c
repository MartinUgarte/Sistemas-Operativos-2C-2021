...
boot_alloc(uint32_t n)
{
	...
	char *next_va_free = ROUNDUP(nextfree + n, PGSIZE);
	physaddr_t next_pa_free = PADDR(next_va_free);
	physaddr_t max_physical_mem = PGSIZE * npages;
	if (next_pa_free > max_physical_mem)
		panic("boot_alloc error: there is not enough physical memory");
	result = nextfree;
	nextfree = next_va_free;
	return result;
}
...
mem_init(void)
{
	...
	uint32_t nenvs_size = NENV * sizeof(struct Env);
	envs = (struct Env *) boot_alloc(nenvs_size);
	memset(envs, 0, nenvs_size);
...
page_init(void)
{
	...
	uintptr_t page_number = PGNUM(first_free_pa);
	size_t i;
	for (i = 1; i < npages; i++) {
		if (i == PGNUM(MPENTRY_PADDR))
			continue;
		if (i < PGNUM(IOPHYSMEM) || page_number < i) {
			pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *current_page = page_free_list;
	page_free_list = current_page->pp_link;
	current_page->pp_link = NULL;

	if (alloc_flags & ALLOC_ZERO) {
		memset(page2kva(current_page), 0, PGSIZE);
	}
	return current_page;
}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("Page free error: unable free page");
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (size_t i = va_aligned; i < end_aligned; i += PGSIZE) {
		pte = pgdir_walk(env->env_pgdir, (void *) i, 0);
		if (!pte || i >= ULIM || !(*pte & PTE_P) || (*pte & perm) != perm) {
			user_mem_check_addr = (uint32_t) va > i ? (uint32_t) va : i;  // esto porque falla buggyhello con la direccion va no con va_aligned
			return -E_FAULT;
		}
	}
	return 0;
