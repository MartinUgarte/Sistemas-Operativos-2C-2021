...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	// Seteamos ring 3
	tf->tf_cs |= 3;
	// seteamos permiso de interrupcion e IOLP 0
	tf->tf_eflags |= (FL_IOPL_0 | FL_IF);
	// Asiganamos el tf al env
	env->env_tf = *tf;
	return 0;
}
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	// envid2env hace tambien la validacion de permisos con el ultimo param
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	result = page_insert(env->env_pgdir, page, (void *) va, perm);
	if (result < 0) {
		page_free(page);
		return result;
	}
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	if (((perm & PTE_W) == PTE_W) && ((*pte & PTE_W) != PTE_W))
		return -E_INVAL;
	result = page_insert(dst_env->env_pgdir, page, dstva, perm);
	if (result < 0) {
		page_free(page);
		return result;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	// el recpeto no esta esperando un mensaje
