...
copy_shared_pages(envid_t child)
{
	...
	for (size_t pg_idx = 0; pg_idx < PDX(USTACKTOP); pg_idx++) {
		pde_t pde = uvpd[pg_idx];
		if (pde & PTE_P) {
			for (size_t pte_idx = 0; pte_idx < NPTENTRIES; pte_idx++) {
				uintptr_t pte_address =
				        (uintptr_t) PGADDR(pg_idx, pte_idx, 0);
				int perm = uvpt[PGNUM(pte_address)] & PTE_SYSCALL;
